...
# main.py — Streamlit UI for the Natural Language → SQL app (login, chat, results, charts).
        ...
        with tab_signup:
            ...
            with _c3:
                ...
                with st.form("signup_form", clear_on_submit=True):
                    ...
                    password = st.text_input("Choose a password", type="password", key="signup_password", help="Minimum 6 characters.")
...
def _app_ui() -> None:
    ...
    if run:
        ...
        st.session_state["messages"] = (st.session_state.get("messages") or []) + [{"role": "user", "content": query.strip()}]
        ...
        st.session_state["messages"] = (st.session_state.get("messages") or []) + [{"role": "assistant", "content": assistant_msg}]


# Streamlit entry point: loads env, sets up the page, and chooses login vs app UI.
# Takes nothing in, returns nothing.
def main() -> None:
    if load_dotenv is not None:
        # Load `.env` so local development has the same variables as production.
